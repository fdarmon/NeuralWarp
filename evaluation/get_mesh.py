...
def get_surface_high_res_mesh(occ, resolution=100, refine_bb=True, cams=None, masks=None,
                              bbox_size=1.):
    ...
    for i, pnts in enumerate(tqdm((torch.split(points, 100000, dim=0)))):
    ...
    mesh_low_res.export("tmp/before.ply")
    if cams is not None:
        if masks is not None:
            ...
            face_mask = np.array([True])
        face_mask = face_mask & mesh_filtering.visible_mask(mesh_low_res, cams, nb=0).cpu().numpy()
        mesh_low_res.update_faces(face_mask)

    components = mesh_low_res.split(only_watertight=False)
    areas = np.array([c.area for c in components], dtype=np.float)
    mesh_low_res = components[areas.argmax()]
    mesh_low_res.export("tmp/after.ply")
    ...
    for i, pnts in enumerate(tqdm(torch.split(points, chunk_size, dim=0))):
        pnts = pnts.cuda()
        ...
        occ_vals = occ(tmp).detach()
        z.append(occ_vals.cpu().numpy())
